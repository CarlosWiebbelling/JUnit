...
/**
 * This class is the controller of the edit menu screen:
 * <code>EditMenu.fxml</code>. Provides add, remove and replace method to edit
 * the menu.
 *
 *
 * @author Mingxuan Mei
...
public class EditMenuController implements Initializable {
     ...
     * <code>TableView editMenuTable</code>, populate the TableView with current
    ...
    /**
     * <code>add</code> add the given item with content in all specified fields
     * into the database and the <code>TableView editMenuTable</code>.
     *
    ...
    public void add(ActionEvent e) {
        ...
        if (addName.getText().isEmpty() || addPrice.getText().isEmpty() || addType.getValue().isEmpty() || !isDouble(addPrice.getText())) {
            ...
            try {
                while (mainResultSet.next()) {
                    Menu name = new Menu(mainResultSet.getString("name"), mainResultSet.getString("type"), mainResultSet.getDouble("price"), 0, "", "");
                    if(mainResultSet.getString("name").equals(addName.getText())) {
                        sff = 0;
                        break;
                    }
                }
            } catch (SQLException ex) {
                Logger.getLogger(EditMenuController.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            if(sff == 1) {
                ...
                status.setText("Item inserted successfully");
            } else {
                status.setText("Unable to add item to menu. Item already inserted");
            }
        }
    }

    /**
     * <code>replace</code> Replace the selected item with content in all
     * specified fields and write the changes into the database.
     *
    ...
    /**
     * <code>remove</code> Remove the selected item from
     * <code>TableView editMenuTable</code> and database.
     *
    ...
    public void remove(ActionEvent e) {
        if (editMenuTable.getSelectionModel().isEmpty()) {
            ...
            SQLiteConnection.write(query);
            status.setText("Item removed successfully");
        }
    }

    /**
     * <code>isDouble</code> This method check if the input price is a double
     * number.
     *
     * @param str The input string is the text get from
     * <code>TextField addPrice</code>.
     *
     * @return Boolean This returns the boolean value of whether the input
