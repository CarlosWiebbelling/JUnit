...
/**
 * This class is the controller of the edit staff screen:
 * <code>EditStaff.fxml</code>. Provides add, remove and replace method to edit
 * the staff accounts.
 *
 * @author Mingxuan Mei
 * @version 1.0
...
public class StaffController implements Initializable {
     ...
     * <code>TableView staffTable</code>, populate the TableView with current
    ...
    /**
     * <code>add</code> add the given staff account with content in all
     * specified fields into the database and the
     * <code>TableView staffTable</code>.
     *
    ...
    public void add(ActionEvent e) {
        ...
        if (staffFirst.getText().isEmpty() || staffLast.getText().isEmpty() || staffUsername.getText().isEmpty() || staffPassword.getText().isEmpty()) {
            ...
            try {
                while (mainResultSet.next()) {
                    Staff name = new Staff(mainResultSet.getString("FirstName"), mainResultSet.getString("LastName"), mainResultSet.getString("Username"), mainResultSet.getString("Password"));
                    if(mainResultSet.getString("Username").equals(staffUsername.getText())) {
                        status.setText("Unable to insert new record! Existing username");
                        stt = 0;
                    }
                }
            ...
            if(stt == 1) {
                ...
                status.setText("New user inserted successfully");
            } else {
                status.setText("Unable to insert new record! Existing username");
            }
        }
    ...
    /**
     * <code>replace</code> Replace the selected staff account with content in
     * all specified fields and write the changes into the database.
     *
    ...
    /**
     * <code>remove</code> Remove the selected staff account from
     * <code>TableView staffTable</code> and database.
     *
    ...
    /**
     * <code>activityLog</code> Switch the screen to
     * <code>ActivityLog.fxml</code> (allows the manager to view records of
     * staffs and manager activities).
     *
...
//            Main.thestage.setScene(scene3);
            //New Form
            Parent window3 = (AnchorPane) FXMLLoader.load(getClass().getResource("/application/ActivityLog.fxml"));
